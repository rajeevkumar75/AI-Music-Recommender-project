    ...
    for i in indices[0]:
        ...
        if filters:
            ...
            if "genres" in filters and filters["genres"]:
                if "genre" in row and row["genre"] not in filters["genres"]:
                    continue
            
            # Duration filter
            if "duration_range" in filters:
                min_duration, max_duration = filters["duration_range"]
                duration_minutes = row.get("duration_ms", 0) / 60000
                if not (min_duration <= duration_minutes <= max_duration):
                    continue
            
            # Popularity filter
            if "popularity_range" in filters and "popularity" in music.columns:
                min_pop, max_pop = filters["popularity_range"]
                if not (min_pop <= row.get("popularity", 0) <= max_pop):
                    continue
            
            # Year filter
            if "year_range" in filters and "year" in row:
